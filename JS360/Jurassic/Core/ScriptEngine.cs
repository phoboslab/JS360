...
namespace Jurassic
{
...
#endif
    {
            ...
            get
            {
                lock (lowPrivilegeEnvironmentLock)
                {
                    if (lowPrivilegeEnvironmentTested == false)
                    {
                        ...
                        lowPrivilegeEnvironment = false;//!System.Security.SecurityManager.IsGranted(permission);
        ...
        /// </summary>
        [NonSerialized]
        internal ReflectionEmitModuleInfo ReflectionEmitInfo = null;
        ...
        public List<ClrFunction> registeredClrFunctions;
        public HashSet<String> compiledBinders;

#if !XBOX
        /// <summary>
        /// Compile the JavaScript source code into an Assembly
        /// </summary>
        /// <param name="code">JavaScript source</param>
        /// <param name="path">path to save the assembly to</param>
        /// <param name="identity">name of the assembly</param>
        public void CompileToFile(string code, string path, string identity)
        {
            CompileToFile(new StringScriptSource(code), path, identity);
        }
        ...
        /// <param name="path">path to save the assembly to</param>
        /// <param name="identity">name of the assembly</param>
        public void CompileToFile(ScriptSource source, string path, string identity)
        {
            string fileName = string.Format("{0}.dll", identity);
            ...
            ReflectionEmitInfo.AssemblyBuilder = System.Threading.Thread.GetDomain().DefineDynamicAssembly(
                new System.Reflection.AssemblyName(identity), System.Reflection.Emit.AssemblyBuilderAccess.Save, path);
            
            // Create a dynamic module.
            ReflectionEmitInfo.ModuleBuilder = ReflectionEmitInfo.AssemblyBuilder.DefineDynamicModule("Module", fileName);
            ReflectionEmitInfo.TypeBuilder = ReflectionEmitInfo.ModuleBuilder.DefineType("Generated", System.Reflection.TypeAttributes.Public | System.Reflection.TypeAttributes.Class);

            // Enable IL Generation
            Compiler.CompilerOptions options = CreateOptions();
            options.EmitModule = true;
            ...
            methodGen.IsMainClass = true;
            methodGen.Parse();
            methodGen.Optimize();
            methodGen.GenerateCode();

            ReflectionEmitInfo.TypeBuilder.CreateType();


            // Generate Binders
            this.compiledBinders = new HashSet<string>();
            var typeBuilder = this.ReflectionEmitInfo.ModuleBuilder.DefineType("Binders", System.Reflection.TypeAttributes.Public | System.Reflection.TypeAttributes.Class);
           
            foreach (ClrFunction f in registeredClrFunctions)
                f.CompileBinder(typeBuilder);

            typeBuilder.CreateType();

            ReflectionEmitInfo.AssemblyBuilder.Save(fileName);
        }
#endif


        /// <summary>
        /// Load all compiled Binders and methods from an Assembly
        /// </summary>
        /// <param name="name">The assembly to load from</param>
        public void LoadFromAssembly(String name)
        {
            System.Reflection.Assembly assembly = System.Reflection.Assembly.Load(name);
            Jurassic.Compiler.GeneratedMethod.LoadCacheFromType(assembly.GetType("Generated"), "Func_");

            Type binders = assembly.GetType("Binders");
            foreach (ClrFunction f in registeredClrFunctions)
            {
                f.LoadBinderFromType(binders);
            }
        }

        /// <summary>
        /// Register a ClrFunction, so its Binders can be compiled or loaded again from an Assembly
        /// </summary>
        /// <param name="func">ClrFunction to register</param>
        public void RegisterClrFunction(ClrFunction func)
        {
            if (registeredClrFunctions == null)
                registeredClrFunctions = new List<ClrFunction>();

            registeredClrFunctions.Add(func);
        }
    }
}
